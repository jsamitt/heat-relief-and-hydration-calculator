...
document.addEventListener('DOMContentLoaded', () => {
  ...
  inputs.forEach(id => {
    ...
    // Validation
    if (highTemp <= 0 || humidity < 0 || cloudCover < 0 || windSpeed < 0 || !workRate || !acclimatized) {
      output.innerHTML = '<p class="placeholder">Please complete all fields.</p>';
      return;
    }

    // 1. WBGT Calculation (NIOSH outdoor formula)
    const tempC = (highTemp - 32) * 5/9;
    const solarIrradiance = 800 * (1 - cloudCover / 100); // W/m²
    const wbgt = 0.7 * tempC + 0.2 * (humidity / 100 * tempC) + 0.1 * (solarIrradiance / 100);
    ...
    let resultsHTML = `
      ...
      <p class="note" style="margin-top:1rem; font-size:0.9rem; color:#555;">
        <em>Based on NIOSH 2016 Criteria and OSHA proposed heat rule. WBGT uses peak conditions. Adjust for clothing, wind, and individual health.</em>
      </p>
    `;
